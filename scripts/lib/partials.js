...
const { renderSeoHead } = require("./seo");
...
const FONT_STYLESHEET =
  "https://fonts.googleapis.com/css2?family=Anton&family=Montserrat:ital,wght@1,600&family=Nunito:ital,wght@0,400;0,600;0,700;1,400&display=swap";
...
<head>
    ...
    <title>${escapeHtml(title)}</title>${seoMeta}
    ...
    <link rel="apple-touch-icon" href="${faviconHref}">${headExtra}`;
}
        ...
        <ul class="nav-menu">
          ...
          <li class="mobile-donate">
            <a href="${donateHref}" class="donate-btn">
              <span aria-hidden="true"><img src="${heartSrc}" alt="" style="width: 14px;"></span> Donate
            </a>
          </li>
        </ul>
...
<section class="footer-cta-section${extraClass ? ` ${extraClass}` : ""}">
    <div class="footer-cta-container">
        <div class="footer-cta-banner">
            <img src="${resolveAsset(depth, cta.backgroundImage, { cloudinaryWidth: 1400 })}" alt="${escapeHtml(cta.backgroundImageAlt || "")}" class="footer-cta-bg" loading="lazy" decoding="async">
            ...
            <div class="footer-cta-card">
                <div class="footer-cta-content">
                    ...
                    <a href="${escapeHtml(cta.buttonUrl)}" class="footer-cta-button">
                        <span aria-hidden="true"><img src="${heartSrc}" alt="" style="width: 14px;"></span>
                        <span>${escapeHtml(cta.buttonLabel)}</span>
                    </a>
                </div>
                
                <div class="footer-cta-qr">
                    <img src="${resolveAsset(depth, cta.qrImage)}" alt="${escapeHtml(cta.qrImageAlt || "QR Code")}" width="120" height="120">
                </div>
            </div>
        </div>
        <hr>
...
function renderPage({
  ...
  const scriptTags = scripts
    .map((script) => `<script src="${resolveAsset(depth, script)}" defer></script>`)
    .join("\n    ");
