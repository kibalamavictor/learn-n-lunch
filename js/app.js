document.addEventListener("DOMContentLoaded", () => {
  ...
  /* =======================
     ...
     CSS drives continuous motion; JS only pauses for swipe/scroll.
  ==========================*/
  const partnersMarquee = document.querySelector(".partners-marquee");
  const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
  const partnersMobileQuery = window.matchMedia("(max-width: 900px)");

  if (partnersMarquee) {
    let resumeTimer = null;

    const canAnimate = () =>
      partnersMobileQuery.matches && !prefersReducedMotion.matches;

    const pauseMotion = () => {
      if (!canAnimate()) return;
      partnersMarquee.classList.add("is-paused");
    ...
    const resumeMotion = () => {
      ...
      resumeTimer = setTimeout(() => {
        if (!canAnimate()) return;
        partnersMarquee.scrollLeft = 0;
        partnersMarquee.classList.remove("is-paused");
      }, 1200);
    };
    ...
    partnersMarquee.addEventListener("pointerdown", pauseMotion, { passive: true });
    partnersMarquee.addEventListener("touchstart", pauseMotion, { passive: true });
    partnersMarquee.addEventListener("wheel", pauseMotion, { passive: true });
    partnersMarquee.addEventListener("pointerup", resumeMotion, { passive: true });
    partnersMarquee.addEventListener("pointercancel", resumeMotion, { passive: true });
    partnersMarquee.addEventListener("touchend", resumeMotion, { passive: true });
    partnersMarquee.addEventListener("touchcancel", resumeMotion, { passive: true });
    partnersMarquee.addEventListener(
      "scroll",
      () => {
        if (partnersMarquee.classList.contains("is-paused")) {
          resumeMotion();
        }
      },
      { passive: true }
    ...
    if (typeof partnersMobileQuery.addEventListener === "function") {
      partnersMobileQuery.addEventListener("change", sync);
      prefersReducedMotion.addEventListener("change", sync);
    } else {
      partnersMobileQuery.addListener(sync);
      prefersReducedMotion.addListener(sync);
    }
...
// DONATE_PAGE — only run when donate controls exist
const buttons = document.querySelectorAll(".amount-option");
const customInput = document.getElementById("custom-amount-input");

if (buttons.length && customInput) {
